...
import scipy.signal  # Добавлено для кросс-кореляції


class MultiMicSynchronizer:
    def __init__(self, device_list, samplerate=48000, blocksize=1024, save_to_file=True, output_dir="recordings"):
    ...
    def align_signals(self, sig1, sig2):
        """Выровнять два сигнала по максимальной корреляции"""
        # Привести к float32 для корреляции
        sig1 = sig1.astype(np.float32)
        sig2 = sig2.astype(np.float32)
        corr = scipy.signal.correlate(sig1, sig2, mode='full')
    ...
    def combine_files_to_stereo(self, filepath_left, filepath_right, output_filepath):
        """Об'єднати два монофоничні WAV-файли у стереофонічний WAV з постсинхронизацией"""
        ...
        with wave.open(filepath_left, 'rb') as left_wav, wave.open(filepath_right, 'rb') as right_wav:
            ...
            # --- Синхронизация по кросс-корреляції ---
            left_aligned, right_aligned = self.align_signals(left_array, right_array)

            # Обрезаем до однакової довжини
            ...
            stereo_array = np.empty((min_len * 2,), dtype=dtype)
            stereo_array[0::2] = left_aligned
            stereo_array[1::2] = right_aligned
        ...
        print(f"Stereo WAV файл з постсинхронизацией збережено: {output_filepath}")
...
def create_dual_mic_setup(save_files=True, output_directory="recordings"):
    ...
    # Select two devices
    device1 = input_devices[2]
    device2 = input_devices[1]
    ...
    # Устанавливаем частоту дискретизации 48000 Гц
    chosen_samplerate = 44100

    # Проверяем, поддерживают ли оба устройства 48000 Гц
    try:
        sd.check_input_settings(device=device1, channels=1, samplerate=chosen_samplerate)
        sd.check_input_settings(device=device2, channels=1, samplerate=chosen_samplerate)
    except Exception as e:
        print(f"Warning: One of the devices does not support 48000 Hz. Error: {e}")
        print("Trying 44100 Hz instead.")
        chosen_samplerate = 44100
        try:
            sd.check_input_settings(device=device1, channels=1, samplerate=chosen_samplerate)
            sd.check_input_settings(device=device2, channels=1, samplerate=chosen_samplerate)
        except Exception as e:
            print("Error: Devices do not support 48000 or 44100 Hz.")
            return None

    print(f"Using sample rate: {chosen_samplerate} Hz")
    print(f"Device {device1} default rate: {device1_info['default_samplerate']} Hz")
    print(f"Device {device2} default rate: {device2_info['default_samplerate']} Hz")
